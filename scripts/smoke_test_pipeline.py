"""Run real initialized models over representative Phase 7 images."""
...
def main() -> int:
    ...
    for image in args.images:
        try:
            ...
            statuses = ", ".join(
                f"{name}={status.status}" for name, status in result.module_status.items()
            )
            print(
                image,
                f"review={result.review_status}",
                f"risk={result.final.risk_level}",
                statuses,
